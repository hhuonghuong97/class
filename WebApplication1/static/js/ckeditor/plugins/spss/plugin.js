...
function isSpssFile(fileName) {
    var ext = fileName.substring(fileName.lastIndexOf('.') + 1);
    if (ext == "sav" || ext == "spv" || ext == "pdf") {
        return true;
    } else {
        alert("Vui lòng chọn file spss");
        return false;
    }
    return true;
}
...
CKEDITOR.plugins.add('spss', {
    icons: 'spss',
    init: function (editor) {
        CKEDITOR.dialog.add('spssDialog', function (editor) {
            return {
                title: 'Thêm file spss',
                minWidth: 400,
                minHeight: 100,
                contents: [
                    {
                        ...
                        elements: [
                            {
                                type: 'file',
                                id: 'file',
                                validate: CKEDITOR.dialog.validate.notEmpty("Vui lòng chọn file spss"),
                                onChange: function (event) {
                                    var filename = this.getValue();
                                    var isValid = false;
                                    if (filename == '') {
                                        isValid = false;
                                    } else if (isSpssFile(filename)) {
                                        isValid = true;
                                    } else {
                                        isValid = false;
                                        this.setValue('');
        ...
        editor.addCommand('insertSpss', new CKEDITOR.dialogCommand('spssDialog'));
        editor.ui.addButton('spss', {
            label: 'Upload file spss',
            command: 'insertSpss',
            toolbar: 'insert'
        });
    }
